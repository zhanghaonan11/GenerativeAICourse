...
有时候，当我们听到一个词太多次时，就不会去追问它到底意味着什么。什么是人工智能？要解释这个问题，让我们回顾一下历史。
...
在过去，为了让计算机完成某项任务——展示一条 Instagram 帖子、让我们在亚马逊上购买商品、计算我们的税款——我们必须通过给计算机明确的指令来编程。因此，我们作为人类一步一步地编程应用程序来解决这些问题。这意味着，我们先作为人类解决了问题，然后把这些指令交给计算机。
...
而有了人工智能，我们不再需要先解决问题，而是通过给机器提供大量不同的示例或数据来教它解决问题。
...
我经常举一个非常基础的例子：假设我们有一个应用程序，根据房屋大小预测房价。我们可以自己编写逻辑，比如说房价总是大小乘以 3。但如果我们不自己写公式，而是向计算机展示数百万个房价和房屋大小的示例或数据点呢？计算机会从所有这些数据点中得出结论："哦，实际上不只是大小乘以 3，可能是大小乘以 1.5。"即使你向我这个人类展示数百万个房价和房屋大小的示例，我也会得出比简单猜测房价等于大小乘以 3 更好的公式，但当然人类无法查看数百万个示例。
...
因此，人工智能就是教计算机从大型数据集或示例中学习。在我们想要分析数据并解决问题的情况下，人工智能表现出色。以 Instagram 信息流推荐为例：没有人工智能，我们可以让人类来解决问题（即决定给你展示什么），或者我们可以让人工智能查看你的浏览历史，并基于这些数据决定给你展示什么。
...
我想定义一下什么是人工智能模型。因为到目前为止，我们一直在高层次上讨论——我们说人工智能是教计算机从大型数据集中解决问题。
...
幕后发生了什么？纯粹是数学。不要害怕数学，因为数学函数不过是接受输入并产生输出的东西。有些函数将输入加倍，所以如果你输入 2，你会得到 4（y = 2x）。有些函数反转输入，所以如果你输入 2，你会得到 -2（y = -x）。这些都是数学函数。
...
**这个数学公式就是人工智能模型。** 因此，有一个预测房价的人工智能模型——那是一个预测房价的数学函数。输入是房屋大小，输出是房价。有一个人工智能模型根据你的浏览活动在你的 Instagram 信息流中放置相关内容——那是另一个专门针对 Instagram 浏览数据训练的数学函数。输入是浏览活动，输出是 Instagram 帖子。但同样，这些都是计算机基于数百万个数据示例得出的函数。
...
更准确地说，在实践中，人类会选择基础数学函数。例如，我们知道房价与大小成正比（房子越大，价格越贵），所以我们可以使用线性函数来建模，如（y = ax + b）。然后计算机查看所有输入和输出对的示例（房价及其大小），然后确定 a 和 b 的最合适值是什么（这些被称为**参数**）。以前，我们只是猜测房价是其大小的 3 倍（y = 3x + 0），但有了人工智能，我们让它查看多个示例并得出准确的 a 和 b 值。
...
监督学习是训练人工智能模型的第一种也是最常见的方法。我们通过给机器提供一组标记数据来教它解决问题——不是任意数据，而是**标记数据**。让我们以房屋大小预测器为例。为了教导计算机，我们给它一系列房屋大小和房价的输入/输出对，所有这些数据都是标记好的。它说，"对于这个房屋大小，这是价格，"我们有数百万这样的数据。计算机不断尝试不同的公式，与标记数据集进行比较，直到得出完美的公式。这就是我们所说的训练人工智能或机器学习模型——这就是为什么训练人工智能模型成本高昂，因为计算机需要针对数百万个数据集不断尝试不同的公式。但现在我们有了一个根据大小预测房价的人工智能模型。
...
如果我们想构建一个人工智能模型，一个能够预测图片是狗还是猫的数学函数，我们给它数百万张标记好的狗和猫的图片。所有这些都是标记好的，因此它被称为**监督**学习。我们人类完成了标记数据的艰苦工作，因此在监督机器从这些数据中学习。
...
无监督学习是指数据只有输入而没有输出的情况。我们不是试图预测某些东西，比如电子邮件是否是垃圾邮件；相反，我们只是在数据中发现模式。购物数据是最容易想到的例子。计算机可以从这些海量数据中发现模式，并得出结论：购买面包的顾客通常也会购买牛奶。这些数据没有标记或没有输出——它只是购买行为，包含"这个顾客买了鸡蛋，这个买了燕麦，这个买了牛奶"等信息，从所有这些数据中，它发现模式。
...
所以监督学习是："这是房屋大小和价格，学习根据大小预测价格。"无监督学习是："这是购物数据，找出有趣的模式。"
...
在亚马逊上，产品推荐（"购买了 x 的顾客也购买了 y"）就是无监督机器学习模型的一个例子。下次你在 Netflix 上时，你会被无监督机器学习模型轰炸，告诉你"看过这个系列的人也看了这个。"
...
强化学习不是从标记数据中学习或在未标记数据中寻找模式，而是通过试错来学习。这就像训练一只狗。当狗做了好事，你就给它一个奖励。随着时间的推移，狗会学会哪些行为会得到奖励，哪些不会。
...
在强化学习中，机器学习模型采取行动，并根据这些行动获得奖励或惩罚。例如，人工智能国际象棋选手使用强化学习。它进行了数百万场比赛，获胜的走法获得正奖励，失败的走法获得负奖励。随着时间的推移，机器发现了人类棋手从未考虑过的策略。自动驾驶车辆是另一个例子。自动驾驶汽车因保持在车道内、保持安全距离和到达目的地而获得奖励。
...
当我们谈论奖励时，当然我们不是给计算机一个奖品。奖励只是一个数值，告诉算法一个行动是好还是坏。正数表示好的行动，负数表示坏的行动。
...
在强化学习中，我们不是预先给人工智能一个数据集来学习。相反，我们把它放在一个环境中（真实的或模拟的），在那里它采取行动，观察结果，并获得奖励或惩罚。
...
对于像 ChatGPT 这样的人工智能模型，该模型使用监督模型进行训练，在数百万标记数据（如书籍或网站）上进行训练。然后模型使用强化学习，为给定的提示生成多个可能的响应，这些响应根据其有用性或准确性由人类或另一个人工智能系统进行评分。模型将这些评分作为奖励接收。
...
你知道人工智能自 20 世纪 50 年代就已经存在了吗？问题是，为什么它突然成为主流？我们很早就有了 Google 翻译，那就是人工智能。我们有信用卡欺诈检测，那也是人工智能。我们很早就有了 Instagram 信息流算法。为什么它突然火了起来？

Sure, products like ChatGPT became viral, but here's the actual reason: In order for AI to work, you need 3 things:

当然，像 ChatGPT 这样的产品确实走红了，但这里有真正的原因：为了让人工智能工作，你需要 3 样东西：
...
像 OpenAI 这样的公司所做的是创建了一个通用模型，并向公众开放，任何人都可以使用。这些模型非常好地理解自然语言，事实证明，能够理解自然语言的模型是通用模型，这意味着它们可以用于解决许多日常用例，如编写代码、客户支持等。
...
在过去，你必须培训一支机器学习工程师大军，并投资大量基础设施来训练你自己的人工智能模型。

So what happened now is, every company can now become an AI company.

所以现在发生的是，每家公司现在都可以成为一家人工智能公司。
